...
namespace caro {
    ...
    namespace {
        ...
        MenuItemView MakeToggleItem(const std::string& id, const std::string& label, bool value, bool selected, Language lang) {
            MenuItemView item;
            item.id = id;
            item.label = label;
            item.value = FormatOnOff(value, lang);
    ...
    MenuView BuildSettingsMenuView(const MenuContext& context) {
        ...
        view.items.push_back(MakeToggleItem("music_enabled", SelectText(lang, "Nhac", "Music"), context.appSettings.musicEnabled, context.settingsSelected == 1, lang));
        view.items.push_back(MakeSliderItem("sound_volume", SelectText(lang, "Am luong", "Sound Volume"), context.appSettings.soundVolume, context.settingsSelected == 2));
        view.items.push_back(MakeSliderItem("music_volume", SelectText(lang, "Nhac luong", "Music Volume"), context.appSettings.musicVolume, context.settingsSelected == 3));
