...
namespace caro {
    ...
    namespace {
        ...
        MenuItemView MakeSlotItem(const SaveSlotInfo& slot, bool selected, const std::string& saveNameDraft, Language lang) {
            MenuItemView item;
            item.id = "save_slot_" + std::to_string(slot.slotIndex);
            item.label = FormatSaveSlotLine(slot, lang);
            item.value = saveNameDraft;
            item.kind = MenuItemKind::SaveSlot;
            item.selected = selected;
            item.hint = SelectText(lang, "Enter: luu vao o nay", "Enter: save to this slot");
            return item;
        }

        MenuItemView MakeBackItem(bool selected, Language lang) {
            MenuItemView item;
            item.id = "back";
            item.label = SelectText(lang, "Quay lai", "Back");
    ...
    MenuView BuildSaveMenuView(const MenuContext& context) {
        ...
        view.subtitle = SelectText(lang, "Ten ban luu hien tai: ", "Current save name draft: ") + context.saveNameDraft;
        view.message = context.statusMessage;
        view.footerHint = SelectText(
            lang,
            "W/S: di chuyen | Enter: luu | ESC: quay lai",
            "W/S: move | Enter: save | ESC: back"
        );
        ...
        view.items.push_back(MakeBackItem(context.saveSelected == static_cast<int>(context.saveSlots.size()), lang));
        return view;
    }
