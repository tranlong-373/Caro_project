...
namespace caro {
    ...
    namespace {
        ...
        std::string GetDefaultSaveName(Language lang) {
            return SelectText(lang, "Ban luu moi", "New Save");
        }
    ...
    void InitializeMenuContext(MenuContext& context, const GameSettings& initialSettings) {
        ...
        context.newGameDraft.playerOName = (initialSettings.gameMode == GameMode::PVE)
            ? GetBotName(initialSettings.aiDifficulty, initialSettings.language)
            : GetDefaultPlayer2Name(initialSettings.language);

        context.saveSlots.clear();
        context.saveNameDraft = GetDefaultSaveName(initialSettings.language);
    ...
    void SyncNewGameDraftFromAppSettings(MenuContext& context) {
        ...
        context.newGameDraft.playerXName = GetDefaultPlayer1Name(context.appSettings.language);

        if (context.newGameDraft.settings.gameMode == GameMode::PVE) {
            context.newGameDraft.playerOName = GetBotName(context.newGameDraft.settings.aiDifficulty, context.appSettings.language);
        }
        else {
            context.newGameDraft.playerOName = GetDefaultPlayer2Name(context.appSettings.language);
        }
    }
    ...
    void SetMenuNewGameNames(MenuContext& context, const std::string& playerX, const std::string& playerO) {
        context.newGameDraft.playerXName = playerX.empty()
            ? GetDefaultPlayer1Name(context.appSettings.language)
            : playerX;
        ...
        if (context.newGameDraft.settings.gameMode == GameMode::PVE) {
            ...
            context.newGameDraft.playerOName = playerO.empty()
                ? GetDefaultPlayer2Name(context.appSettings.language)
                : playerO;
        }
    }
    ...
    void OpenSaveMenuScreen(MenuContext& context, const std::string& suggestedSaveName) {
        ...
        context.saveSelected = 0;
        context.saveNameDraft = suggestedSaveName.empty()
            ? GetDefaultSaveName(context.appSettings.language)
            : suggestedSaveName;
        ClearMenuStatusMessage(context);
    }
